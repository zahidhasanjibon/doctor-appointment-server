import { useContext, useEffect, useState } from "react";
import { AiOutlineClose, AiOutlineMenu } from "react-icons/ai";
import { BsMoon, BsSun } from "react-icons/bs";
import {
  Link,
  NavLink,
  useLocation,
  useMatch,
  useNavigate
} from "react-router-dom";
import logo from "../../assests/img/icons8-tooth-cracked-96.png";

import { authContext } from "../authentication/AuthContext";
import "./navbar.css";
export default function Navbar() {
  const [showMenu, setShowMenu] = useState(false);
  const [toggle, setToggle] = useState(false);
  const [activeMenu, setActiveMenu] = useState("home");

  const { user, isLoading, logOut } = useContext(authContext);

  const { pathname } = useLocation();
  const match = useMatch(pathname.slice(1));
  useEffect(() => {
    if (match) {
      if (match.pathname.slice(1).length > 0) {
        setActiveMenu(match.pathname.slice(1));
      }
    }
  }, [match]);

  const navigate = useNavigate();
  const handleLogOut = () => {
    logOut()
      .then(() => {
        localStorage.removeItem("jwttoken");
        navigate("/login");
      })
      .catch((err) => {
        console.log(err);
      });
  };

  // toggle theme

  // set active menu function
  const handleActiveMenu = (val) => {
    setActiveMenu(val);
  };

  return (
    <div className="container mx-auto">
      <div className="w-11/12 mx-auto pt-2">
        <nav className="navbar-nav grid items-center grid-cols-5">
          <div
            onClick={() => setShowMenu(false)}
            className="brand md:col-span-1 col-span-4"
          >
            <NavLink to="/">
              <img
                onClick={() => setActiveMenu("home")}
                className="logo cursor-pointer"
                src={logo}
                alt="logo"
              />
            </NavLink>
          </div>
          <div
            onClick={() => setShowMenu(!showMenu)}
            className="block lg:hidden col-end-6 ml-auto cursor-pointer"
          >
            {!showMenu ? (
              <AiOutlineMenu size={30} />
            ) : (
              <AiOutlineClose size={30} />
            )}
          </div>
          <div
            onClick={() => setShowMenu(false)}
            className={`${
              showMenu ? "col-start-1 col-span-4" : "hidden"
            } menu-bar py-4 md:py-0 bg-white ml-4 relative z-10 md:ml-0  lg:col-span-2 xl:col-span-3  lg:block`}
          >
            <ul className="md:flex md:justify-around xl:px-24">
              <li onClick={() => handleActiveMenu("home")}>
                <Link
                  className={`${activeMenu === "home" ? "active" : undefined}`}
                  to="/"
                >
                  Home
                </Link>
              </li>
   
          <li onClick={() => handleActiveMenu("services")}>
            <Link
              to="/services"
              className={`${
                activeMenu === "services" ? "active" : undefined
              }`}
            >
              services
            </Link>
          </li>
          <li onClick={() => handleActiveMenu("appontment")}>
            <Link
              to="/appointment"
              className={`${
                activeMenu === "appointment" ? "active" : undefined
              }`}
            >
              appointment
            </Link>
          </li>
           
            {
              (
                user?.uid && (
                  <li onClick={() => handleActiveMenu("reviews")}>
                  <Link
                    to="/reviews"
                    className={`${
                      activeMenu === "reviews" ? "active" : undefined
                    }`}
                  >
                    My Reviews
                  </Link>
                </li>
                )
              )
            }
             {
              user?.uid && ( <li onClick={() => handleActiveMenu("addservice")}>
              <Link
                to="/addservice"
                className={`${activeMenu === "addservice" ? "active" : undefined}`}
              >
                Add Service
              </Link>
            </li>)
             }
              <li onClick={() => handleActiveMenu("blog")}>
                <Link
                  to="/blog"
                  className={`${activeMenu === "blog" ? "active" : undefined}`}
                >
                  Blog
                </Link>
              </li>
            </ul>
          </div>

          <div
            className={`${
              showMenu ? "col-start-1 col-span-4" : "hidden"
            } avatar-wrapper bg-white  ml-4 relative z-10 md:ml-0 pb-10 md:pb-0  lg:col-span-2 xl:col-span-1 md:col-span-2 lg:flex`}
          >
            {!isLoading && (
              <>
                {user?.uid ? (
                  <>
                    <div className="tooltip">
                      <span className="tooltip-text">
                        {user?.displayName || "not/available"}
                      </span>
                      <img
                        className="avatar-logo-user"
                        src={
                          user?.photoURL ||
                          "data:image/png;base64,"
                        }
                        alt=""
                      />
                    </div>

                    <button
                      className="btn btn-primary btn-outline btn-sm"
                      onClick={handleLogOut}
                    >
                      logout
                    </button>
                  </>
                ) : (
                  <>
                    <NavLink
                      className="btn btn-primary btn-oyline btn-sm"
                      to="/login"
                    >
                      Log in
                    </NavLink>
                  </>
                )}
              </>
            )}

            {!toggle ? (
              <span
                className="cursor-pointer"
                onClick={() => setToggle(!toggle)}
              >
                <BsSun size={25} />
              </span>
            ) : (
              <span
                className="cursor-pointer"
                onClick={() => setToggle(!toggle)}
              >
                <BsMoon size={25} />
              </span>
            )}
          </div>
        </nav>
      </div>
    </div>
  );
}
